...
\subsection{Archive-Level Audit}
...
\subsection{Visual Evidence Generation}
...
\subsection{Mesh Quality and Pattern-Graph Metrics}
...
\subsection{Pattern-Mesh Consistency Score}
...
\subsection{Retrieval and Reconstruction Baselines}
...
\subsection{Render-Image Retrieval Baseline}
...
\subsection{Hard-Generalization Protocols}
...
\subsection{Full Download and Verification}
...
\subsection{Category Coverage and Pattern Complexity}
...
\subsection{Mesh Quality Benchmark}
...
\subsection{Pattern Structure Benchmark}
...
\subsection{Pattern-Mesh Consistency}
...
\subsection{Render-to-Pattern Retrieval Baseline}
...
\subsection{Render Retrieval Error Analysis}
...
\subsection{Hard-Generalization Analysis}
...
The proposed consistency score is not a replacement for downstream geometric metrics such as Chamfer distance, normal consistency, IoU, or simulation stability. Instead, it acts as a pre-training gate. A model trained on paired patterns and meshes can silently fail if segmentation files are misaligned with mesh vertices or if a small number of samples lack OBJ evidence. The full-data score shows that this dataset is unusually clean, but it also demonstrates why a reproducible evidence gate should precede model training.
...
The motivation for this study includes traditional costume modeling, but the current corpus should not be overclaimed as a traditional-garment dataset. Garments such as the Vietnamese \textit{ao dai} require long tunic panels, side slits, collar structures, sleeve variations, layered trousers, and motif alignment rules that are not fully represented by the existing template families. Table~\ref{tab:traditionalgap} and Fig.~\ref{fig:traditionalgap} summarize the transfer gap.
...
\section{Reproducibility and Extended Diagnostics}\label{sec:reproducibility}

\subsection{Benchmark Split Details}\label{sec:splits}

Table~\ref{tab:splits} gives the exact stratified split used by the retrieval and reconstruction baselines. The split is deterministic with a fixed random seed and preserves the category distribution of the full 22,450-sample production subset.
...
\subsection{Per-Category Retrieval Diagnostics}\label{sec:percat}
...
\subsection{Ablation Matrix and Error Counts}\label{sec:ablation}
...
\section{Retrieval Confusion Matrices}\label{sec:confusion}
...
\section{Metric Definitions and Reproducibility Notes}\label{sec:metrics}
...
\section{Artifact Inventory}\label{sec:artifacts}

Table~\ref{tab:artifacts} summarizes the concrete artifacts produced by the benchmark. The separation into experiment directories is intentional: the initial evidence audit, quantitative mesh-pattern benchmark, structured-feature retrieval, render-image retrieval, and manuscript-specific diagnostic figures can be rerun or inspected independently.
...
\section{Evaluation Protocol Card}\label{sec:protocol}

The benchmark is intended to separate evidence quality from model ability. Table~\ref{tab:protocolcard} states the recommended interpretation of each metric group. This prevents a common error in garment reconstruction papers: treating high category retrieval accuracy as if it were already evidence of accurate sewing-pattern reconstruction.
...
This schema is intentionally stricter than what is needed for generic garment retrieval. Traditional costume modeling risks producing visually plausible but culturally incorrect assets if side slits, collars, layered trousers, textile motifs, and expert labels are absent. The current benchmark therefore supplies the measurement protocol, while a traditional-garment corpus must supply the missing cultural and construction evidence.
...
The audited dataset is available from the Zenodo record for the Dataset of 3D Garments with Sewing Patterns: \url{https://zenodo.org/records/5267549}. The experiment outputs include the verification manifest, per-sample audit table, category summaries, missing-evidence report, benchmark metrics, and generated figures.
